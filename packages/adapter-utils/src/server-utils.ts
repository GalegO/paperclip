...
export const DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE = [
  ...
  "",
  "You have access to a 4-level persistent memory hierarchy. Use it to store knowledge that should survive across heartbeats and sessions.",
  "",
  "### Memory Levels",
  "",
  "- **L4 Global** (GLOBAL.md) — Instance-wide: user preferences, OS defaults, communication habits",
  "- **L3 Company** (COMPANY_<id>.md) — Organization-wide: business rules, intranet links, culture guidelines",
  "- **L2 Project** (PROJECT_<id>.md) — Project-specific: conventions, assets, repo patterns, workflow decisions",
  "- **L1 Agent** (AGENT_<id>.md) — Agent-personal: recurring bugs you've seen, CSS tricks, domain expertise",
  "",
  "### How to Write Memories",
  "",
  "Use the Paperclip API to manage memories:",
  "",
  "```bash",
  "# Add a memory",
  "curl -X POST $PAPERCLIP_API_URL/api/memories \\",
  "  -H \"Authorization: Bearer $PAPERCLIP_API_KEY\" \\",
  "  -H \"Content-Type: application/json\" \\",
  "  -d '{\"target\":\"global\",\"content\":\"User prefers concise responses\"}'",
  "",
  "# Add a company-level memory",
  "curl -X POST $PAPERCLIP_API_URL/api/memories \\",
  "  -H \"Authorization: Bearer $PAPERCLIP_API_KEY\" \\",
  "  -H \"Content-Type: application/json\" \\",
  "  -d '{\"target\":\"company\",\"companyId\":\"<company-id>\",\"content\":\"Use internal wiki for docs\"}'",
  "",
  "# Add a project-level memory",
  "curl -X POST $PAPERCLIP_API_URL/api/memories \\",
  "  -H \"Authorization: Bearer $PAPERCLIP_API_KEY\" \\",
  "  -H \"Content-Type: application/json\" \\",
  "  -d '{\"target\":\"project\",\"projectId\":\"<project-id>\",\"content\":\"Always run lint before commit\"}'",
  "",
  "# Add an agent-level memory",
  "curl -X POST $PAPERCLIP_API_URL/api/memories \\",
  "  -H \"Authorization: Bearer $PAPERCLIP_API_KEY\" \\",
  "  -H \"Content-Type: application/json\" \\",
  "  -d '{\"target\":\"agent\",\"agentId\":\"<agent-id>\",\"content\":\"PostgreSQL uses port 5433 in this env\"}'",
  "```",
  "",
  "### When to Write Memories",
  "",
  "- **L4 Global:** User preferences, communication style, default tools, recurring patterns across all companies",
  "- **L3 Company:** Business rules, team conventions, internal URLs, compliance requirements",
  "- **L2 Project:** Architecture decisions, repo-specific workflows, deployment patterns, test conventions",
  "- **L1 Agent:** Personal lessons learned, domain expertise, recurring issues you've solved",
  "",
  "### Guidelines",
  "",
  "- Write memories proactively — do not wait to be asked",
  "- Keep entries concise and actionable",
  "- Use the correct level — do not put project-specific info in global memory",
  "- Memories are injected at the start of each heartbeat session (read-only during session)",
  "- You can add/replace/remove memories at any time via the API",
  "",
  "{{context.paperclipMemoryMarkdown}}",
].join("\n");
